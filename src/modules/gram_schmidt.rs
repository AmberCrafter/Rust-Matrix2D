...
use crate::{Matrix2D, Zero, One, Epsilon, MatrixError, ScalarOperation};
    ...
    Vec<T>: ScalarOperation<T>
{
    ...
    fn dot_vector(v1: &Vec<T>, v2: &Vec<T>) -> T {
        v1.iter().zip(v2.iter()).fold(T::zero(), |acc, (x1, x2)| acc + *x1 * *x2)
    }
}
    ...
    fn qr_decomposition(&self) -> Result<MatrixQR<T>, MatrixError> {
        ...
        for i in 0..et.len() {
            ...
            for j in 0..at.shape.0 {
                tmp.push(<Matrix2D<T> as GramSchmidt<T>>::dot_vector(&at.data[j], &et[i]));
            }
            r.push(tmp);
        }
